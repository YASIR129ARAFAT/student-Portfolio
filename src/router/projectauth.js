...
// const { router2, global_enrollment } = require("./loginauth");
...
router.post("/add_project", middlewares.verifyUser, (req, res, next) => {
  // console.log(global_enrollment + "d");
  ...
  var projectDes = req.body.projectDescription;
  var sql = `insert into projects  (projectTitle,projectDescription ,gitHubLink, instructor, enrolment_no,isVerified ) values ( "${title}","${projectDes}","${githubLink}", "${instructor}", "${
    req.user.enroll_no
  }",${1})`;
